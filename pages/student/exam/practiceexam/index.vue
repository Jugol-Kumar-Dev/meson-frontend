...
const {
  ...
  status: status,
  refresh
} = await useLazyAsyncData(`student-practice-${authUser?.id}-exams`, () => $fetch(`/get-practice-exams`, {
...
<!--      <p class="text-sm font-semibold text-secondary-600">Lorem ipsum dolor sit amet, consectetur adipisicing elit.</p>-->
    ...
    <div class="mt-10">
      ...
      <div v-if="status === 'success' && exams?.data?.length" class="flex flex-col gap-5">
        <div class="bg-white flex items-start lg:items-center flex-col lg:flex-row justify-between shadow-lg p-5" v-for="exam in exams?.data" :key="exam">
          <div>
          ...
          <div>
            <NuxtLink :to="`/student/exam/practiceexam/details?course_id=${exam?.course_id}&exam_id=${exam.id}`"
