...
const {
  ...
  status,
  refresh
} = await useLazyAsyncData(`student-practice-${authUser?.id}-exams`, () => $fetch(`/get-given-live-exams`, {
...
<template>
  <div class="w-full h-full min-h-screen">
    <div class="bg-white w-full flex items-center justify-center flex-col py-5 shadow-md">
      <h2 class="font-bold text-xl">Given Live Exams</h2>
<!--      <p class="text-sm font-semibold text-secondary-600">Lorem ipsum dolor sit amet, consectetur adipisicing elit.</p>-->
    ...
    <div class="mt-10">
      ...
      <div v-if="status !== 'pending' && exams?.data?.length" class="flex flex-col gap-2">
        <!-- Header -->
        ...
        <!-- Exam Results List -->
        <div v-for="exam in exams?.data" :key="exam.id" class="bg-white shadow-md rounded-lg py-2 px-5 flex-col lg:flex-row flex items-center justify-between gap-2 lg:gap-6">

          <!-- Exam Name -->
          <div class="flex-1">
            <p class="font-semibold text-base capitalize text-primary-700">{{ exam?.name }}</p>
          </div>

          <!-- Exam Duration and Total Questions -->
          <div class="flex-1 text-center">
            <p class="font-bold text-lg text-secondary-800">{{ exam?.duration }} min</p>
            <p class="font-semibold text-sm text-secondary-500">{{ exam?.total_q }} Questions</p>
          </div>
          ...
          <div class="flex-1 text-right">
            <NuxtLink
                :to="`/student/exam/result/details?exam_id=${exam.mock_user_id}`"
