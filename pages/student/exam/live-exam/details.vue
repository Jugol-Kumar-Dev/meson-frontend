...
const {
  ...
  status,
  refresh
} = await useLazyAsyncData(`student-${authUser?.id}-live-single-exams-${useRoute().query?.exam_id}`,
    () => $fetch(`/get-live-exam-details/${useRoute().query?.exam_id}`, {
...
const getExamToken = async () => {
  ...
  const {data, error, status} = await useApiFetch(`/check-live-mocktest/${useRoute().query?.exam_id}`, {
    method: 'POST',
    cache: "no-cache"
  })

  if(error.value?.data){
    alert('Not Valid Time For Given The Exam. Or Already Given...')
  }else{
