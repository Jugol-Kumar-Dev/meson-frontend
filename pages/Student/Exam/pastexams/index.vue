...
const {
  ...
  status: status,
  refresh
} = await useLazyAsyncData(`student-${authUser?.id}-exams`, () => $fetch(`/get-my-past-exams`, {
...
<template>
  <div class="w-full h-full min-h-screen">
    <div class="bg-white w-full flex items-center justify-center flex-col py-5">
      <h2 class="font-bold text-xl">Past Exams</h2>
    ...
    <div class="mt-10">
      <div class="flex flex-col gap-5">
        <div class="bg-white flex items-center justify-between shadow-lg p-5" v-for="exam in exams?.data" :key="exam">
          ...
          <div>
            <NuxtLink :to="`/student/exam/pastexams/details?course_id=${exam?.course_id}&exam_id=${exam.id}`"
